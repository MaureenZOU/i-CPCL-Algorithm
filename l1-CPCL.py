...
def updateCooperative(McObject, MuObject, XtObject, p1, p2, epoch):
...
def Algorithm(clusterNum, seedPointNum, epoch, combine):
    ...
    for k in range(0, epoch):
        ...
        for i in range(0, len(dataSet)):
            ...
            for data in Su:
                updateCooperative(seedPoints[winLoc], data, dataSet[i], winLoc, matrixSeedDict[data.positionNum], k)
                edgeMatrix[winLoc][matrixSeedDict[data.positionNum]] = edgeMatrix[winLoc][
                                                                           matrixSeedDict[data.positionNum]] + 1

            for data in Sp:
                updatePenalize(seedPoints[winLoc], data, dataSet[i], winLoc, matrixSeedDict[data.positionNum], k)
                edgeMatrix[winLoc][matrixSeedDict[data.positionNum]] = edgeMatrix[winLoc][
                                                                           matrixSeedDict[data.positionNum]] - 1
    ...
    print("Execute Time: " + str(start-end)+" sec")
...
fileName = "./animation/l1-CPCL_Animation_s2.csv"
...
fileName = "./center/l1-CPCL_center_s2.csv"
...
fileName = "./epoch/l1-CPCL_RMSE_s2_"+str(seedNum)+".csv"
writeFile(RMSE, fileName)

fileName = "./cpu/l1-CPCL_RMSE_s2_"+str(seedNum)+"pyt.csv"
...
line = "l1-CPCL_RMSE_s2_"+str(seedNum)+": "+ str(distance/(len(seedPoints)*d))+"\n"
fileName = "result.log"
