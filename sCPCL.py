...
def updateCooperative(McObject, MuObject, XtObject, p1, p2, edgeMatrix):
    ...
    l = 0.001 + 0.0001 * np.absolute(edgeMatrix[p1][p2])
    #print(l)
    #l = 0.001
...
def mergePoint(seedPoints, loc1, loc2):
    seedPoints[loc1] = seed((seedPoints[loc1].prePoint + seedPoints[loc2].prePoint) / 2,
                            ...
                            (seedPoints[loc1].seedWinNum + seedPoints[loc2].seedWinNum) / 2, 0, loc1)

    print(seedPoints[loc2].trace)
...
def Algorithm(clusterNum, seedPointNum, epoch, combine):
    ...
    for k in range(0, epoch):
        ...
        RMSE.append([rmse/len(seedPoints)])

        finalEpoch = k
        print("epoch: "+str(k))

    return seedPoints, finalEpoch
...
seedCount = 1
RMSE = []
seedPoints, finalEpoch = Algorithm(clusterNum, seedNum, epoch, combine)
seedList = []
center = []
centerSeed = 0
...
fileName = "./rmse/sCPCL_RMSE_s1"+str(seedNum)+".csv"
...
# line = str(clusterNum) + ": cluster number = "+str(clusterNum)+" seed Points Number = "+str(seedNum)+" epoch = "+ str(finalEpoch) + "\n"
# fileName = "new.txt"
